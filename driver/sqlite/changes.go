...
func (s *rds) recordChange(ctx context.Context, tx *sql.Tx, urn storage.URN, action string, at time.Time) error {
...
func (s *rds) loadChangeCursor(ctx context.Context, consumer string, kind storage.Kind, after time.Time) (changeCursor, error) {
	...
	if _, err := s.db.ExecContext(ctx,
		`INSERT OR IGNORE INTO change_cursors(consumer, kind, created_at, seq) VALUES (?, ?, ?, ?)`,
