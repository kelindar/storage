...
// rds represents a PostgreSQL storage layer for resources.
type rds struct {
	db       *database
...
func Open(dsn string, registry storage.Registry) (storage.Storage, error) {
	...
	store, err := newStore(&database{DB: db}, registry, true)
	if err != nil {
		_ = db.Close()
		return nil, err
	}
...
// New creates a PostgreSQL storage layer over an existing database handle.
// The returned storage does not close db.
func New(db *sql.DB, registry storage.Registry) (storage.Storage, error) {
	if db == nil {
		return nil, fmt.Errorf("storage: database is required")
	}
	return newStore(&database{DB: db}, registry, false)
}

func newStore(db *database, registry storage.Registry, own bool) (storage.Storage, error) {
...
func queryOrder(field string) string {
	...
	default:
		...
		return "data #>> string_to_array('" + field + "', '.')" + suffix
	}
}
...
func queryFilterByJSON(path string, values []string) (string, []any) {
	...
	expression := "data #>> string_to_array(?, '.')"
